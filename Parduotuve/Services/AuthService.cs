...
public class AuthService
{
    ...
    private async Task<User?> FetchUser()
    {
        ...
        if (!res.Success || string.IsNullOrEmpty(res.Value)) return null;
        User? user = await _userRepo.GetBySessionIdAsync(res.Value);
    ...
    private async Task<Result> InjectSessionId(User user)
    {
        ...
        if (id == null) return Result.Err($"Session ID creation failed");
    ...
    {
        ProtectedBrowserStorageResult<string> res = await _localStorage.GetAsync<string>(SessionIdKey);
        if (res.Success) await _userRepo.DeleteSessionAsync(res.Value ?? "");
    ...
    public async Task<Result<User>> LoginAsync(string username, string password)
    {
        ...
        if (user == null) return Result<User>.Err($"Invalid username");

        if (user.Password != password) return Result<User>.Err($"Invalid password");

        Result injectResult = await InjectSessionId(user);

        if (!injectResult) return Result<User>.Err(injectResult.Message);
    ...
    {
        User newUser = new() { Username = username, Password = password, Role = role, Email = email };
